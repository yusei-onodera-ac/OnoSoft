...
- ポップアップ右上の **「常に表示」ボタン**(📍→📌)を押すと、他のウィンドウをクリックしても
  閉じなくなる。複数の項目を落ち着いて削除・整理したいときに便利(閉じるには
  もう一度ボタンを押すか、`Esc` キー、またはホットキーの再押下)
...
- **設定画面**(トレイメニュー→「設定」)でテーマ(ダーク/ライト)・アクセントカラー・文字サイズを変更可能
...
(`TrayIconService`, `BackgroundMessageWindow`, `NativeMethods`, `ThemeApplier`, `UpdateChecker`)。
